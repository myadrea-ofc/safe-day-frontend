...
import 'package:safety_apps/widgets/dropdown/no_unit/dropdown_no_exca.dart';
...
class FormP2HExcavator extends StatefulWidget {
  @override
  _FormP2HExcavatorPageState createState() => _FormP2HExcavatorPageState();
}

class _FormP2HExcavatorPageState extends State<FormP2HExcavator> {
  TextEditingController nama = TextEditingController();
  TextEditingController waktu = TextEditingController();
  TextEditingController hm = TextEditingController();
  ...
  List<String> _getMissingFields() {
    ...
    if (hm.text.trim().isEmpty) missing.add(fHM);
    if (lokasi_kerja.text.trim().isEmpty) missing.add(fLokasi);
    if (jabatan == null) missing.add(fJabatan);
    if (department == null) missing.add(fDepartment);
    if (perusahaan == null) missing.add(fPerusahaan);
    if (noUnit == null) missing.add(fNoUnit);
  ...
  void initState() {
    ...
    nama.addListener(() {
      ...
      if (lokasi_kerja.text.trim().isNotEmpty) {
        _clearMissing(fLokasi);
      }
      if (hm.text.trim().isNotEmpty) {
        _clearMissing(fHM);
      }
    });
  }
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  CheckboxOther(
                    selected: jabatan,
                    options: ["Operator Excavator", "Pengawas", "Trainer"],
                    onChanged: (v) {
                      setState(() => jabatan = v);
                  ...
                  CheckboxOther(
                    selected: perusahaan,
                    options: ["PT BBP", "PT MCM", "Mine Contractor"],
                    onChanged: (v) {
                  ...
                  LabelText(
                    fNoUnit,
                    showError: _missingFields.contains(fNoUnit),
                  ),
                  DropdownNoUnitExca(
                    value: noUnit,
                    onChanged: (v) {
                      setState(() => noUnit = v);
                      _clearMissing(fNoUnit);
                    },
                  ),
                  ...
                  LabelText(
                    fLokasi,
                    showError: _missingFields.contains(fLokasi),
                  ),
                  InputField(
                  ...
                  LabelText("Item Pemerikasaan P2H Unit Excavator"),
                  ...
                  SizedBox(
                    ...
                    child: Container(
                      ...
                      child: ElevatedButton(
                        onPressed: _isSubmitting ? null : submitP2HExca,
  ...
  void submitP2HExca() async {
    ...
    bool ok = false;
    try {
      ok = await P2HExcaService.submitP2HExca(
        nama: nama.text,
        jabatan: jabatan ?? "",
        department: department ?? "",
        perusahaan: perusahaan ?? "",
        tanggal: tanggal.text,
        noLambungUnit: noUnit ?? "",
        hmSekarang: hm.text,
        waktuP2H: waktu.text,
        lokKerja: lokasi_kerja.text,
        ...
        opsiStandarKeselamatan4: opsistandardkeselamatan4 ?? "",
