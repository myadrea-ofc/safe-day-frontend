...
class _FormInspeksiPlantPageState extends State<FormInspeksiPlant> {
  ...
  Map<String, dynamic> _buildInspeksiPlantPayload() {
    return {
      'nama': nama.text,
      'nrp': nrp.text,
  ...
  List<String> _buildInspeksiPlantFilePaths() {
    if (kIsWeb) return [];
  ...
  Future<bool> _submitInspeksiPlantOnce(Map<String, dynamic> payload) async {
    return await InspeksiPlantService.submitInspeksiPlant(
      ...
      _submitTimeout,
      onTimeout: () {
        debugPrint("SUBMIT INSPEKSI PLANT TIMEOUT");
        return false;
      },
    );
  }
  ...
  void submitInspeksiPlant() async {
    ...
    final payload = _buildInspeksiPlantPayload();
    final filePaths = _buildInspeksiPlantFilePaths();
    ...
    try {
      ok = await RetrySubmitHelper.run(
        ...
        action: () => _submitInspeksiPlantOnce(payload),
      );

      if (!ok) {
        await PendingFormHelper.saveInspeksiPlant(
          payload: payload,
          filePaths: filePaths,
        );
        savedToPending = true;
