...
class _FormInspeksiCHPPageState extends State<FormInspeksiCHP> {
  ...
  Map<String, dynamic> _buildInspeksiCHPPayload() {
    return {
      'nama': nama.text,
      'nrp': nrp.text,
  ...
  List<String> _buildInspeksiCHPFilePaths() {
    if (kIsWeb) return [];
  ...
  Future<bool> _submitInspeksiCHPOnce(Map<String, dynamic> payload) async {
    return await InspeksiCHPService.submitInspeksiCHP(
      ...
      _submitTimeout,
      onTimeout: () {
        debugPrint("SUBMIT INSPEKSI CHP TIMEOUT");
        return false;
      },
    );
  }
  ...
  void submitInspeksiCHP() async {
    ...
    final payload = _buildInspeksiCHPPayload();
    final filePaths = _buildInspeksiCHPFilePaths();
    ...
    try {
      ok = await RetrySubmitHelper.run(
        ...
        action: () => _submitInspeksiCHPOnce(payload),
      );

      if (!ok) {
        await PendingFormHelper.saveInspeksiCHP(
          payload: payload,
          filePaths: filePaths,
        );
        savedToPending = true;
