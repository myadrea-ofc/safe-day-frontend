...
class P2HDTService {
  ...
  static const String baseUrl = "http://safety.borneo.co.id/p2h_dt";

  static Future<bool> submitP2HDT({
    ...
    try {
      ...
      if (kIsWeb) {
        ...
        if (filePaths != null && filePaths.isNotEmpty) {
          ...
          for (final path in filePaths.take(5)) {
            ...
            debugPrint("Uploading P2H HEAVY DUTY file $i/${filePaths.length}");
      ...
      debugPrint("SUBMIT P2H HEAVY DUTY STATUS: ${response.statusCode}");
      debugPrint("SUBMIT P2H HEAVY DUTY RESPONSE: ${response.body}");
      ...
      debugPrint("Submit P2H HEAVY DUTY Exception: $e");
      return false;
    } finally {
      client.close();
    }
  ...
  static Future<List<P2HDTModel>> fetchP2HDT() async {
    final token = await _storage.read(key: "jwt_token");
    final deviceId = await _storage.read(key: "device_id");
    ...
    try {
      ...
      return data.map((e) => P2HDTModel.fromJson(e)).toList();
    } catch (e) {
      debugPrint("Error Fetch P2H DT: $e");
      return [];
    }
  }
}
