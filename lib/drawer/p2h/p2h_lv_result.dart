...
class P2HLVResultPage extends StatefulWidget {
  final int? openDetailId;
  const P2HLVResultPage({super.key, this.openDetailId});
  @override
  State<P2HLVResultPage> createState() => _P2HLVResultPageState();
}

class _P2HLVResultPageState extends State<P2HLVResultPage>
    with SingleTickerProviderStateMixin {
  List<P2HLVModel> allData = [];
  List<P2HLVModel> filtered = [];
  ...
  Future<void> loadData() async {
    try {
      final data = await P2HLVService.fetchP2HLV();
      if (!mounted) return;
  ...
  List<P2HLVModel> _applySearchAndDate() {
  ...
  List<ExcelAccess> get _sortedAccessNewest {
    final list = List<ExcelAccess>.from(
      _accessList.where((a) => a.feature == "p2h_lv"),
    );
    list.sort((a, b) => b.createdAt.compareTo(a.createdAt));
    return list;
  }

  List<P2HLVModel> _searchResults(String v) {
    final q = v.toLowerCase().trim();

    String safeLower(String? s) => (s ?? "").toLowerCase().trim();

    if (q.isEmpty) return List<P2HLVModel>.from(allData);
    ...
    int firstIndexRow(P2HLVModel e) {
      final n = safeLower(e.nama);
      final p = safeLower(e.perusahaan);
      final d = safeLower(e.department);
  ...
  Widget _table() {
    ...
    final columns = const [
      ...
      DataColumn2(label: Center(child: Text("Brand Unit")), fixedWidth: 200),
      DataColumn2(label: Center(child: Text("No Lambung LV")), fixedWidth: 200),
      DataColumn2(
        label: Center(child: Text("No HM LV Saat ini")),
        fixedWidth: 240,
      ),
      DataColumn2(label: Center(child: Text("Shift Kerja")), fixedWidth: 150),
      DataColumn2(label: Center(child: Text("Siap Kerja")), fixedWidth: 150),
  ...
  DataRow _rowPremium(P2HLVModel e, int index) {
    ...
    return DataRow(
      ...
      cells: [
        ...
        DataCell(buildResultCellWrap(e.brandUnit)),
        DataCell(buildResultCellWrap(e.noLambungUnit)),
        DataCell(buildResultCellWrap(e.lvSekarang)),
        ...
        return ResultDetailBottomSheet(
          ...
          content: Column(
            ...
            children: [
              ...
              flatBoxP2H("Brand Unit", e.brandUnit),
              const SizedBox(height: 12),

              pairP2H(
                context,
                flatBoxP2H("No Lambung LV", e.noLambungUnit),
                flatBoxP2H("No LV saat ini", e.lvSekarang),
              ),
              const SizedBox(height: 12),

              pairP2H(
                context,
                flatBoxP2H("tanggal Kejadian", formatTanggal(e.tanggal)),
              ...
              pairP2H(
                context,
                flatBoxP2H("Level Oil Engine", e.opsiItem1),
              ...
              pairP2H(
                ...
                flatBoxP2H("Kondisi Aki", e.opsiItem4),
              ),
              const SizedBox(height: 12),
              pairP2H(
                context,
                flatBoxP2H("Air Wiper dan Kondisi Wiper", e.opsiItem5),
                flatBoxP2H("Kondisi Ban", e.opsiItem6),
              ),
              const SizedBox(height: 12),
              pairP2H(
                context,
                flatBoxP2H("Ban Serap", e.opsiItem7),
                flatBoxP2H("Skrup Ban", e.opsiItem8),
              ),
              const SizedBox(height: 12),
              pairP2H(
                context,
                flatBoxP2H("Rem Kaki", e.opsiItem9),
                flatBoxP2H("Rem Tangan", e.opsiItem19),
              ),
              const SizedBox(height: 12),
              pairP2H(
                context,
                flatBoxP2H("Lampu Kerja (Depan & Belakang)", e.opsiItem10),
                flatBoxP2H("Lampu Sein (Kiri & Kanan)", e.opsiItem11),
              ),
              const SizedBox(height: 12),
              pairP2H(
                context,
                flatBoxP2H("Lampu Hazard", e.opsiItem12),
                flatBoxP2H("Alarm Mundur", e.opsiItem13),
              ),
              const SizedBox(height: 12),
              pairP2H(
                context,
                flatBoxP2H("Klakson", e.opsiItem14),
                flatBoxP2H("Keadaan Body Mobil", e.opsiItem15),
              ),
              const SizedBox(height: 12),
              pairP2H(
                context,
                flatBoxP2H("Kaca Spion", e.opsiItem16),
                flatBoxP2H("Panel Engine", e.opsiItem17),
              ),
              const SizedBox(height: 12),
              flatBoxP2H("Kondisi AC", e.opsiItem18),
              ...
              flatBoxP2H("Safety Cone / Segitiga", e.opsiStandardKeselamatan1),
              const SizedBox(height: 12),
              pairP2H(
                context,
                flatBoxP2H(
                  "APAR (alat pemadam api ringan)",
                  e.opsiStandardKeselamatan2,
                ),
                flatBoxP2H("Jack", e.opsiStandardKeselamatan3),
              ),
              const SizedBox(height: 12),
              pairP2H(
                context,
                flatBoxP2H("Kotak P3K", e.opsiStandardKeselamatan4),
                flatBoxP2H(
                  "Seat Belt / Sabuk Pengaman",
                  e.opsiStandardKeselamatan5,
                ),
              ),
              const SizedBox(height: 12),
              ...
              flatBoxP2H("Lampu Atap Kabin", e.opsiStandardMasukTambang1),
              const SizedBox(height: 12),
              pairP2H(
                context,
                flatBoxP2H("Lampu Rotari", e.opsiStandardMasukTambang2),
                flatBoxP2H("Scotlight Reflector", e.opsiStandardMasukTambang3),
              ),
              const SizedBox(height: 12),
              pairP2H(
                context,
                flatBoxP2H("No Lambung Unit", e.opsiStandardMasukTambang4),
                flatBoxP2H("Radio Komunikasi", e.opsiStandardMasukTambang5),
              ),
              const SizedBox(height: 12),
              pairP2H(
                context,
                flatBoxP2H("Buggy Whip 4 Mtr", e.opsiStandardMasukTambang6),
                flatBoxP2H("4WD", e.opsiStandardMasukTambang7),
              ),
