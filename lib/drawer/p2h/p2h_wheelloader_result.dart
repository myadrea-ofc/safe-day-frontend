...
import 'package:safety_apps/models/p2h/p2h_wheelloader.dart';
...
class P2HWheelLoaderResultPage extends StatefulWidget {
  final int? openDetailId;
  const P2HWheelLoaderResultPage({super.key, this.openDetailId});
  @override
  State<P2HWheelLoaderResultPage> createState() =>
      _P2HWheelLoaderResultPageState();
}

class _P2HWheelLoaderResultPageState extends State<P2HWheelLoaderResultPage>
    with SingleTickerProviderStateMixin {
  List<P2HWheelLoaderModel> allData = [];
  List<P2HWheelLoaderModel> filtered = [];
  ...
  Future<void> loadData() async {
    try {
      final data = await P2HWheelLoaderService.fetchP2HWheelLoader();
      if (!mounted) return;
  ...
  List<P2HWheelLoaderModel> _applySearchAndDate() {
  ...
  List<P2HWheelLoaderModel> _searchResults(String v) {
    final q = v.toLowerCase().trim();

    String safeLower(String? s) => (s ?? "").toLowerCase().trim();

    if (q.isEmpty) return List<P2HWheelLoaderModel>.from(allData);
    ...
    int rankRow(P2HWheelLoaderModel e) {
    ...
    int firstIndexRow(P2HWheelLoaderModel e) {
      final n = safeLower(e.nama);
      final p = safeLower(e.perusahaan);
      final d = safeLower(e.department);
  ...
  List<P2HWheelLoaderModel> get pageData {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: ResultAppBar(
        title: "P2H Wheel Loader Results",
  ...
  Widget _table() {
    ...
    final columns = const [
      ...
      DataColumn2(
        label: Center(child: Text("No Lambung Wheel Loader")),
        fixedWidth: 220,
      ),
      DataColumn2(label: Center(child: Text("HM Unit")), fixedWidth: 200),
      DataColumn2(label: Center(child: Text("Lokasi Kerja")), fixedWidth: 170),
  ...
  DataRow _rowPremium(P2HWheelLoaderModel e, int index) {
  ...
  void showDetail(P2HWheelLoaderModel e) {
    showModalBottomSheet(
      ...
      builder: (_) {
        return ResultDetailBottomSheet(
          title: "Detail Laporan P2H Wheel Loader",
          ...
          content: Column(
            ...
            children: [
              ...
              pairP2H(
                context,
                flatBoxP2H("No Lambung WheelLoader", e.noLambungUnit),
              ...
              pairP2H(
                context,
                flatBoxP2H("Reaksi Kemudi", e.opsiItem19),
                flatBoxP2H("Tuas Kemudi", e.opsiItem20),
              ),
              const SizedBox(height: 12),
              pairP2H(
                context,
              ...
              pairP2H(
                ...
                flatBoxP2H("Kondisi Shovel", e.opsiItem24),
              ),
              const SizedBox(height: 12),
              pairP2H(
                context,
              ...
              pairP2H(
                ...
                flatBoxP2H("Kotak PK3", e.opsiItem28),
              ),
              const SizedBox(height: 12),
              pairP2H(
                context,
