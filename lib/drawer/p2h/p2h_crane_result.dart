...
import 'package:safety_apps/models/p2h/p2h_crane.dart';
...
class P2HCraneResultPage extends StatefulWidget {
  final int? openDetailId;
  const P2HCraneResultPage({super.key, this.openDetailId});
  @override
  State<P2HCraneResultPage> createState() => _P2HCraneResultPageState();
}

class _P2HCraneResultPageState extends State<P2HCraneResultPage>
    with SingleTickerProviderStateMixin {
  List<P2HCraneModel> allData = [];
  List<P2HCraneModel> filtered = [];
  ...
  Future<void> loadData() async {
    try {
      final data = await P2HCraneService.fetchP2HCrane();
      if (!mounted) return;
  ...
  List<P2HCraneModel> _applySearchAndDate() {
  ...
  List<ExcelAccess> get _sortedAccessNewest {
    final list = List<ExcelAccess>.from(
      _accessList.where((a) => a.feature == "p2h_crane"),
    );
    list.sort((a, b) => b.createdAt.compareTo(a.createdAt));
    return list;
  }

  List<P2HCraneModel> _searchResults(String v) {
    final q = v.toLowerCase().trim();

    String safeLower(String? s) => (s ?? "").toLowerCase().trim();

    if (q.isEmpty) return List<P2HCraneModel>.from(allData);
    ...
    int firstIndexRow(P2HCraneModel e) {
      final n = safeLower(e.nama);
      final p = safeLower(e.perusahaan);
      final d = safeLower(e.department);
  ...
  List<P2HCraneModel> get pageData {
  ...
  Widget _table() {
    ...
    final columns = const [
      ...
      DataColumn2(
        label: Center(child: Text("No Lambung Crane")),
        fixedWidth: 200,
      ),
      DataColumn2(
        label: Center(child: Text("HM Crane Saat ini")),
        fixedWidth: 200,
      ),
      DataColumn2(label: Center(child: Text("Lokasi Kerja")), fixedWidth: 200),
  ...
  DataRow _rowPremium(P2HCraneModel e, int index) {
    ...
    return DataRow(
      ...
      cells: [
        ...
        DataCell(buildResultCellWrap(e.lokasiKerja)),
  ...
  void showDetail(P2HCraneModel e) {
    showModalBottomSheet(
      ...
      builder: (_) {
        return ResultDetailBottomSheet(
          title: "Detail Laporan P2H Crane",
          ...
          content: Column(
            ...
            children: [
              ...
              pairP2H(
                context,
                flatBoxP2H("No Lambung Crane", e.noLambungUnit),
                flatBoxP2H("HM Crane saat ini", e.hmUnit),
              ),
              const SizedBox(height: 12),
              pairP2H(
                context,
              ...
              pairP2H(
                ...
                flatBoxP2H("Bahan Bakar & Oli Mesin", e.opsiItem2),
              ),
              const SizedBox(height: 12),
              pairP2H(
                context,
                flatBoxP2H("Kondisi Wiper", e.opsiItem3),
                flatBoxP2H(
              ...
              pairP2H(
                context,
                flatBoxP2H(
                  "Kondisi Rem (Kaki Parkir dan Penggerak Boom)",
                  e.opsiItem5,
                ),
                flatBoxP2H(
                  "Kondisi Lampu Rotary, Boom, Stop, Depan, Belakang",
                  e.opsiItem6,
                ),
              ),
              const SizedBox(height: 12),
              pairP2H(
                ...
                flatBoxP2H(
                  "Klakson, Load Indicator, Alarm Swing & Mundur",
                  e.opsiItem8,
                ),
              ),
              const SizedBox(height: 12),
              pairP2H(
                context,
                flatBoxP2H("Overload Alarm", e.opsiItem9),
                flatBoxP2H(
              ...
              pairP2H(
                ...
                flatBoxP2H(
                  "Uji Rem Fisik (Kaki dan Penggerak Boom)",
                  e.opsiItem12,
                ),
              ),
              const SizedBox(height: 12),
              pairP2H(
                ...
                flatBoxP2H("Reaksi Kemudi", e.opsiItem14),
              ),
              const SizedBox(height: 12),
              pairP2H(
                context,
                flatBoxP2H("Tuas Kemudi", e.opsiItem15),
                flatBoxP2H("Sistem Hidrolik dan Kebocoran", e.opsiItem16),
              ),
              const SizedBox(height: 12),
              pairP2H(
                context,
                flatBoxP2H("Silinder Hidrolik", e.opsiItem17),
                flatBoxP2H("Sistem Elektrik", e.opsiItem18),
              ),
              const SizedBox(height: 12),
              pairP2H(
                context,
                flatBoxP2H(
                  "Outtriggers (Jack, Slider, Level Gauge dan Base Pale)",
                  e.opsiItem19,
                ),
                flatBoxP2H("Crawler Tracks dan Link Bolt", e.opsiItem20),
              ),
              const SizedBox(height: 12),
              pairP2H(
                context,
                flatBoxP2H(
                  "Cantelan Pengangkat dan Kancing Penutup",
                  e.opsiItem21,
                ),
                flatBoxP2H(
                  "Kondisi Boom Hoist, Silinder dan Telescopic",
                  e.opsiItem22,
                ),
              ),
              const SizedBox(height: 12),
              pairP2H(
                context,
                flatBoxP2H("Indikator Radius Sudut Boom", e.opsiItem23),
                flatBoxP2H("Limit Pengangkat pada Boom", e.opsiItem24),
              ),
              const SizedBox(height: 12),
              pairP2H(
                context,
                flatBoxP2H("Tabel Pengangkat pada Boom", e.opsiItem25),
                flatBoxP2H(
                  "Seling / Tali Pengangkatan Utama (Diameter Kerusakan)",
                  e.opsiItem26,
                ),
              ),
              const SizedBox(height: 12),
              pairP2H(
                context,
                flatBoxP2H(
                  "Kondisi Seling, Webbing, Rantai Bantu Pengangkatan",
                  e.opsiItem27,
                ),
                flatBoxP2H("Teromol Pengangkatan / Pully", e.opsiItem28),
              ),
              const SizedBox(height: 12),
              pairP2H(
                context,
                flatBoxP2H("Radio Komunikasi", e.opsiItem35),
                flatBoxP2H("SIO / SIMPER", e.opsiItem36),
              ),
              const SizedBox(height: 12),
              pairP2H(
                context,
                flatBoxP2H("Drum Tempat Putaran Seling / Tali", e.opsiItem29),
                flatBoxP2H("Shackle", e.opsiItem30),
              ),
              const SizedBox(height: 12),
              pairP2H(
                context,
                flatBoxP2H("Instrumen Panel", e.opsiItem31),
                flatBoxP2H("Alat Pemadam Api", e.opsiItem32),
              ),
              const SizedBox(height: 12),
              pairP2H(
                context,
                flatBoxP2H("Kotak PK3", e.opsiItem33),
                flatBoxP2H("Sabuk Keselamatan", e.opsiItem34),
              ),
              const SizedBox(height: 12),
              ...
              flatBoxP2H(
                "Apakah anda memiliki Kimper & SIO yang masih berlaku",
                e.kimperBerlaku,
              ),
              const SizedBox(height: 12),
