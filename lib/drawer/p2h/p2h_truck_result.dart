...
import 'package:safety_apps/models/p2h/p2h_truck.dart';
...
class P2HTruckResultPage extends StatefulWidget {
  final int? openDetailId;
  const P2HTruckResultPage({super.key, this.openDetailId});
  @override
  State<P2HTruckResultPage> createState() => _P2HTruckResultPageState();
}

class _P2HTruckResultPageState extends State<P2HTruckResultPage>
    with SingleTickerProviderStateMixin {
  List<P2HTruckModel> allData = [];
  List<P2HTruckModel> filtered = [];
  ...
  Future<void> loadData() async {
    try {
      final data = await P2HTruckService.fetchP2HTruck();
      if (!mounted) return;
  ...
  List<P2HTruckModel> _applySearchAndDate() {
  ...
  List<ExcelAccess> get _sortedAccessNewest {
    final list = List<ExcelAccess>.from(
      _accessList.where((a) => a.feature == "p2h_truck"),
    );
    list.sort((a, b) => b.createdAt.compareTo(a.createdAt));
    return list;
  }

  List<P2HTruckModel> _searchResults(String v) {
    final q = v.toLowerCase().trim();

    String safeLower(String? s) => (s ?? "").toLowerCase().trim();

    if (q.isEmpty) return List<P2HTruckModel>.from(allData);
    ...
    int firstIndexRow(P2HTruckModel e) {
      final n = safeLower(e.nama);
      final p = safeLower(e.perusahaan);
      final d = safeLower(e.department);
  ...
  List<P2HTruckModel> get pageData {
  ...
  Widget _table() {
    ...
    final columns = const [
      ...
      DataColumn2(
        label: Center(child: Text("No Lambung Truck")),
        fixedWidth: 200,
      ),
      DataColumn2(
        label: Center(child: Text("HM Truck saat ini")),
        fixedWidth: 200,
      ),
  ...
  DataRow _rowPremium(P2HTruckModel e, int index) {
  ...
  void showDetail(P2HTruckModel e) {
    showModalBottomSheet(
      ...
      builder: (_) {
        return ResultDetailBottomSheet(
          title: "Detail Laporan P2H Truck",
          ...
          content: Column(
            ...
            children: [
              ...
              pairP2H(
                context,
                flatBoxP2H("No Lambung Truck", e.noLambungUnit),
              ...
              pairP2H(
                context,
                flatBoxP2H("Alat Pelambat / Retarder Control", e.opsiItem19),
                flatBoxP2H(
              ...
              pairP2H(
                context,
                flatBoxP2H("Reaksi Kemudi", e.opsiItem21),
                flatBoxP2H("Kemudi Darurat", e.opsiItem22),
              ),
              const SizedBox(height: 12),
              pairP2H(
                context,
                flatBoxP2H("Sistem Hidrolik", e.opsiItem23),
                flatBoxP2H("Silinder Hidrolik", e.opsiItem24),
              ),
              const SizedBox(height: 12),
              pairP2H(
                context,
                flatBoxP2H("Mesin", e.opsiItem25),
                flatBoxP2H("Sistem Elektrik", e.opsiItem26),
              ),
              const SizedBox(height: 12),
              pairP2H(
                context,
              ...
              pairP2H(
                ...
                flatBoxP2H("Radio Komunikasi", e.opsiItem30),
              ),
              const SizedBox(height: 12),
              pairP2H(
                context,
                flatBoxP2H("Safety Cone", e.opsiItem31),
                flatBoxP2H("Ganjal Ban", e.opsiItem32),
              ),
              const SizedBox(height: 12),
              pairP2H(
                context,
              ...
              pairP2H(
                context,
                flatBoxP2H("Jack", e.opsiStandarKeselamatan2),
                flatBoxP2H("Safety Cone / Segitiga", e.opsiStandarKeselamatan3),
              ),
              const SizedBox(height: 12),
