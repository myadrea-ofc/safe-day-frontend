...
class P2HWaterTruckResultPage extends StatefulWidget {
  final int? openDetailId;
  const P2HWaterTruckResultPage({super.key, this.openDetailId});
  @override
  State<P2HWaterTruckResultPage> createState() =>
      _P2HWaterTruckResultPageState();
}

class _P2HWaterTruckResultPageState extends State<P2HWaterTruckResultPage>
    with SingleTickerProviderStateMixin {
  List<P2HWaterTruckModel> allData = [];
  List<P2HWaterTruckModel> filtered = [];
  ...
  Future<void> loadData() async {
    try {
      final data = await P2HWaterTruckService.fetchP2HWaterTruck();
      if (!mounted) return;
  ...
  List<P2HWaterTruckModel> _applySearchAndDate() {
  ...
  List<ExcelAccess> get _sortedAccessNewest {
    final list = List<ExcelAccess>.from(
      _accessList.where((a) => a.feature == "p2h_water_truck"),
    );
    list.sort((a, b) => b.createdAt.compareTo(a.createdAt));
    return list;
  }

  List<P2HWaterTruckModel> _searchResults(String v) {
    final q = v.toLowerCase().trim();

    String safeLower(String? s) => (s ?? "").toLowerCase().trim();

    if (q.isEmpty) return List<P2HWaterTruckModel>.from(allData);
    ...
    int rankRow(P2HWaterTruckModel e) {
    ...
    int firstIndexRow(P2HWaterTruckModel e) {
      final n = safeLower(e.nama);
      final p = safeLower(e.perusahaan);
      final d = safeLower(e.department);
  ...
  List<P2HWaterTruckModel> get pageData {
                            ...
                            ResultDateFilterBar(
                              ...
                              onExportExcel: () async {
                                ...
                                await exportExcelCurrentFilter(
                                  ...
                                  canCurrentUserDownloadExcel:
                                      _canCurrentUserDownloadExcel,
                                  type: ExportType.p2h_water_truck,
                                  range: _presetRange(_datePreset),
                                  onNoAccess: () =>
  ...
  Widget _table() {
    ...
    final columns = const [
      ...
      DataColumn2(
        label: Center(child: Text("No Lambung Water Truck")),
        fixedWidth: 220,
      ),
  ...
  DataRow _rowPremium(P2HWaterTruckModel e, int index) {
    ...
    return DataRow(
      ...
      cells: [
        ...
        DataCell(buildResultCellWrap(e.nrp)),
  ...
  void showDetail(P2HWaterTruckModel e) {
    showModalBottomSheet(
      ...
      builder: (_) {
        return ResultDetailBottomSheet(
          title: "Detail Laporan P2H Water Truck",
          ...
          content: Column(
            ...
            children: [
              ...
              pairP2H(
                context,
                flatBoxP2H("No Lambung Water Truck", e.noLambungUnit),
              ...
              pairP2H(
                ...
                flatBoxP2H("Alarm Mundur", e.opsiItem12),
              ),
              const SizedBox(height: 12),
              pairP2H(
                context,
                ...
                flatBoxP2H("Cermin Pandang Belakang & Sisi", e.opsiItem14),
              ),
              const SizedBox(height: 12),
              pairP2H(
                context,
                ...
                flatBoxP2H("Suhu Mesin", e.opsiItem16),
              ),
              const SizedBox(height: 12),
              pairP2H(
                context,
                flatBoxP2H("Klakson", e.opsiItem17),
                flatBoxP2H("Uji Rem Fisik", e.opsiItem18),
              ),
              const SizedBox(height: 12),
              pairP2H(
                context,
              ...
              pairP2H(
                ...
                flatBoxP2H("Sistem Hidrolik", e.opsiItem22),
              ),
              const SizedBox(height: 12),
              pairP2H(
                context,
                flatBoxP2H("Silinder Hidrolik", e.opsiItem23),
                flatBoxP2H("Mesin", e.opsiItem24),
              ),
              const SizedBox(height: 12),
              pairP2H(
                context,
                flatBoxP2H("Sistem Elektrik", e.opsiItem25),
                flatBoxP2H("Kondisi Tangki", e.opsiItem26),
              ),
              const SizedBox(height: 12),
              pairP2H(
                context,
              ...
              pairP2H(
                context,
                flatBoxP2H("Kotak PK3", e.opsiItem29),
                flatBoxP2H("Sabuk Keselamatan", e.opsiItem30),
              ),
              const SizedBox(height: 12),
              pairP2H(
                context,
                flatBoxP2H("Radio Komunikasi", e.opsiItem31),
                flatBoxP2H("Safety Cone", e.opsiItem32),
              ),
              const SizedBox(height: 12),
              pairP2H(
                context,
                flatBoxP2H("Ganjal Ban", e.opsiItem33),
                flatBoxP2H("SIMPER", e.opsiItem34),
              ),
              const SizedBox(height: 12),
              ...
              flatBoxP2H("APAR (alat pemadam api ringan)", e.opsiKeselamatan1),
              const SizedBox(height: 12),
              pairP2H(
                context,
                flatBoxP2H("Kotak P3K", e.opsiKeselamatan2),
                flatBoxP2H("Rigger Tersedia", e.opsiKeselamatan3),
              ),
