...
_StatusKind _kind(String? v) {
  ...
  if (t == "tidak berfungsi" || t == "rusak") return _StatusKind.bad;
...
class P2HGraderResultPage extends StatefulWidget {
  @override
  State<P2HGraderResultPage> createState() => _P2HGraderResultPageState();
}

class _P2HGraderResultPageState extends State<P2HGraderResultPage> {
  List<P2HGraderModel> allData = [];
  List<P2HGraderModel> filtered = [];
  int rowsPerPage = 10;
  int currentPage = 0;
  bool loading = true;
  ...
  Future<void> loadData() async {
    try {
      final data = await P2HGraderService.fetchP2HGrader();
      setState(() {
        allData = data;
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("Gagal memuat data P2H Grader: $e")),
      );
    }
  }

  // ===== SEARCH (samakan DT: contains + ranking ringan) =====
  List<P2HGraderModel> _searchResults(String v) {
    final q = v.toLowerCase().trim();
    String safeLower(String? s) => (s ?? "").toLowerCase().trim();

    if (q.isEmpty) return List<P2HGraderModel>.from(allData);
    ...
    final results = allData.where((e) {
      ...
      final noUnit = safeLower(e.noLambungUnit);
    ...
    int firstIndexRow(P2HGraderModel e) {
      final n = safeLower(e.nama);
      final r = safeLower(e.nrp);
      final p = safeLower(e.perusahaan);
      final d = safeLower(e.department);
  ...
  List<P2HGraderModel> get pageData {
      ...
      child: ClipRRect(
        ...
        child: TextField(
          ...
          decoration: InputDecoration(
            hintText:
                "Cari nama / NRP / perusahaan / department / jabatan / no unit …",
          ...
          children: [
            ...
            DataTable2(
              ...
              columns: const [
                ...
                DataColumn2(
                  label: Center(child: Text("HM Saat ini")),
                  fixedWidth: 200,
                ),
                DataColumn2(
                  label: Center(child: Text("Lokasi Kerja")),
                  fixedWidth: 150,
                ),
                DataColumn2(
  ...
  DataRow _rowPremium(P2HGraderModel e, int index) {
      ...
      cells: [
        ...
        DataCell(
          Center(
            child: InkWell(
              borderRadius: BorderRadius.circular(14),
              onTap: () => showDetail(e), // isi detail tetap
              child: Container(
                padding: const EdgeInsets.symmetric(
                  horizontal: 14,
                  vertical: 10,
  ...
  // ===== helper pair responsive (biar layout detail aman) =====
  Widget _pair(
  ...
  // ===== reuse section title (samakan DT) =====
  ...
  // ===== flatBox style + status badge otomatis (samakan DT) =====
  ...
  // ===== DOKUMEN DIALOG (samakan DT: header gradien + close gradien) =====
  ...
  // ===== preview & download: logic kamu (tetap), hanya tambah guard null biar aman =====
  ...
  void showDetail(P2HGraderModel e) {
                ...
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _pair(
                        context,
                        _flatBox("No Lambung Grader", e.noLambungUnit),
                        _flatBox("No Grader saat ini", e.hmUnit),
                      ),
                      const SizedBox(height: 12),
                      _pair(
                        context,
                      ...
                      _pair(
                        context,
                        _flatBox("Level Air Radiator & Kebocoran", e.opsiItem1),
                        _flatBox("Level Minyak Rem", e.opsiItem2),
                      ),
                      const SizedBox(height: 12),
                      _pair(
                        context,
                        _flatBox("Level Air Aki", e.opsiItem3),
                        _flatBox("Bahan Bakar & Oli Mesin", e.opsiItem4),
                      ),
                      const SizedBox(height: 12),
                      _pair(
                        context,
                      ...
                      _pair(
                        context,
                        _flatBox("Alarm Mundur", e.opsiItem9),
                        _flatBox(
                      ...
                      _pair(
                        context,
                        _flatBox("Kondisi Mesin dan Suhu Mesin", e.opsiItem13),
                        _flatBox("Klakson", e.opsiItem14),
                      ),
                      const SizedBox(height: 12),
                      _pair(
                        context,
                        ...
                        _flatBox("Reaksi Kemudi", e.opsiItem16),
                      ),
                      const SizedBox(height: 12),
                      _pair(
                        context,
                        _flatBox("Kemudi Darurat", e.opsiItem17),
                        _flatBox("Sistem Hidrolik", e.opsiItem18),
                      ),
                      const SizedBox(height: 12),
                      _pair(
                        context,
                        _flatBox("Silinder Hidrolik", e.opsiItem19),
                        _flatBox("Sistem Elektrik", e.opsiItem20),
                      ),
                      const SizedBox(height: 12),
                      _pair(
                        context,
                        _flatBox("Blade", e.opsiItem21),
                        _flatBox("Grader Blade Turntable", e.opsiItem22),
                      ),
                      const SizedBox(height: 12),
                      _pair(
                        context,
                        _flatBox("Bajak & Gigi", e.opsiItem23),
                        _flatBox("Pengatur Arah", e.opsiItem24),
                      ),
                      const SizedBox(height: 12),
                      _pair(
                        context,
                        _flatBox("Tuas Kemudi / Control Levers", e.opsiItem25),
                        _flatBox(
                      ...
                      _pair(
                        context,
                        _flatBox("Sistem Monitor", e.opsiItem27),
                        _flatBox("Alat Pemadam Api", e.opsiItem28),
                      ),
                      const SizedBox(height: 12),
                      _pair(
                        context,
                        _flatBox("Kotak P3K", e.opsiItem29),
                        _flatBox("Tongkat 4 m & Bendera", e.opsiItem30),
                      ),
                      const SizedBox(height: 12),
                      _pair(
                        context,
                        _flatBox("Sabuk Keselamatan", e.opsiItem31),
                        _flatBox("Radio Komunikasi", e.opsiItem32),
                      ),
                      const SizedBox(height: 12),
                      _flatBox("SIMPER", e.opsiItem33),
                      ...
                      _pair(
                        ...
                        _flatBox("Kotak P3K", e.opsiStandarKeselamatan3),
                      ),
