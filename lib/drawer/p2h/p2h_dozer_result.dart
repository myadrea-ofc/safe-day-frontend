...
// ====== STATUS HELPER (SAMA PERSIS DENGAN CRANE) ======
...
class P2HDozerResultPage extends StatefulWidget {
  @override
  State<P2HDozerResultPage> createState() => _P2HDozerResultPageState();
}

class _P2HDozerResultPageState extends State<P2HDozerResultPage> {
  List<P2HDozerModel> allData = [];
  List<P2HDozerModel> filtered = [];
  int rowsPerPage = 10;
  int currentPage = 0;
  bool loading = true;
  ...
  Future<void> loadData() async {
    try {
      final data = await P2HDozerService.fetchP2HDozer();
      setState(() {
        allData = data;
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("Gagal memuat data P2H Dozer: $e")),
      );
    }
  }

  // ===== SEARCH (SAMA DENGAN CRANE: contains + ranking) =====
  List<P2HDozerModel> _searchResults(String v) {
    final q = v.toLowerCase().trim();
    String safeLower(String? s) => (s ?? "").toLowerCase().trim();

    if (q.isEmpty) return List<P2HDozerModel>.from(allData);
    ...
    final results = allData.where((e) {
      ...
      final brand = safeLower(e.brandUnit);
    ...
    int rankRow(P2HDozerModel e) {
      final fields = [
        ...
        safeLower(e.brandUnit),
      ];
      return fields.map(rankText).reduce((a, b) => a < b ? a : b);
    }

    int firstIndexRow(P2HDozerModel e) {
      final fields = [
        safeLower(e.nama),
        safeLower(e.nrp),
        safeLower(e.perusahaan),
  ...
  List<P2HDozerModel> get pageData {
      ...
      child: ClipRRect(
        ...
        child: TextField(
          ...
          decoration: InputDecoration(
            hintText:
                "Cari nama / NRP / perusahaan / department / jabatan / unit / lokasi / brand …",
          ...
          children: [
            ...
            DataTable2(
              ...
              columns: const [
                ...
                DataColumn2(
                  label: Center(child: Text("No Lambung Dozer")),
                  fixedWidth: 200,
                ),
                DataColumn2(
                  label: Center(child: Text("HM Dozer")),
                  fixedWidth: 200,
                ),
                DataColumn2(
                  label: Center(child: Text("Brand Unit")),
                  fixedWidth: 180,
                ),
                DataColumn2(
  ...
  DataRow _rowPremium(P2HDozerModel e, int index) {
  ...
  // ===== helper responsive pair (SAMA CRANE) =====
  Widget _pair(
  ...
  // ====== DETAIL (LABEL & DATA TIDAK DIUBAH) ======
  void showDetail(P2HDozerModel e) {
                ...
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _pair(
                        context,
                        _flatBox("No Lambung Dozer", e.noLambungUnit),
                        _flatBox("No Dozer saat ini", e.hmUnit),
                      ),
                      const SizedBox(height: 12),
                      _pair(
                        context,
                      ...
                      _pair(
                        ...
                        _flatBox("Oli Mesin", e.opsiItem4),
                      ),
                      const SizedBox(height: 12),
                      _pair(
                        context,
                        _flatBox("Bahan Bakar", e.opsiItem5),
                        _flatBox("Kondisi Wiper", e.opsiItem6),
                      ),
                      const SizedBox(height: 12),
                      _pair(
                        context,
                        ...
                        _flatBox("Kondisi Lampu Body", e.opsiItem8),
                      ),
                      const SizedBox(height: 12),
                      _pair(
                        context,
                      ...
                      _pair(
                        ...
                        _flatBox("Sistem Hidrolika", e.opsiItem16),
                      ),
                      const SizedBox(height: 12),
                      _pair(
                        context,
                        ...
                        _flatBox("Mesin", e.opsiItem18),
                      ),
                      const SizedBox(height: 12),
                      _pair(
                        context,
                        _flatBox("Crawler Tracks", e.opsiItem19),
                        _flatBox("Crawler Track Link Bolts", e.opsiItem20),
                      ),
                      const SizedBox(height: 12),
                      _pair(
                        context,
                        _flatBox("Sistem Elektrik", e.opsiItem21),
                        _flatBox("Blade (Dozer / Grader)", e.opsiItem22),
                      ),
                      const SizedBox(height: 12),
                      _pair(
                        context,
                        _flatBox("Bajak & Gigi / Ripper & Teeth", e.opsiItem23),
                        _flatBox(
                      ...
                      _pair(
                        context,
                        _flatBox("Instrumen Panel", e.opsiItem27),
  ...
  // ===== flatBox (status-aware, label/data tetap) - SAMA CRANE =====
