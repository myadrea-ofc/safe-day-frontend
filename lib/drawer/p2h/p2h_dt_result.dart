...
import 'package:safety_apps/models/p2h/p2h_dt.dart';
import 'package:safety_apps/service/p2h/p2h_dt_service.dart';

// ====== STATUS HELPER (samakan Bus) ======
...
class P2HDTResultPage extends StatefulWidget {
  @override
  State<P2HDTResultPage> createState() => _P2HDTResultPageState();
}

class _P2HDTResultPageState extends State<P2HDTResultPage> {
  List<P2HDTModel> allData = [];
  List<P2HDTModel> filtered = [];
  int rowsPerPage = 10;
  int currentPage = 0;
  bool loading = true;

  // ===== P5M THEME (samakan Bus) =====
  ...
  // ===== LOAD DATA (isi data tetap) =====
  Future<void> loadData() async {
    try {
      final data = await P2HDTService.fetchP2HDT();
      setState(() {
        allData = data;
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("Gagal memuat data P2H Truck: $e")),
      );
    }
  }

  // ===== SEARCH (samakan Bus: contains + ranking ringan) =====
  List<P2HDTModel> _searchResults(String v) {
    final q = v.toLowerCase().trim();
    String safeLower(String? s) => (s ?? "").toLowerCase().trim();

    if (q.isEmpty) return List<P2HDTModel>.from(allData);
    ...
    int firstIndexRow(P2HDTModel e) {
      final n = safeLower(e.nama);
      final p = safeLower(e.perusahaan);
      final d = safeLower(e.department);
      final j = safeLower(e.jabatan);
  ...
  // ===== SEARCH BOX PREMIUM (samakan Bus) =====
          ...
          children: [
            ...
            DataTable2(
              ...
              columns: const [
                ...
                DataColumn2(
                  label: Center(child: Text("HM Unit Saat Ini")),
                  fixedWidth: 200,
                ),
                DataColumn2(
  ...
  DataRow _rowPremium(P2HDTModel e, int index) {
    ...
    Widget statusChip(String v) {
      // khusus "open" biar sama persis Bus
      if (_norm(v) == "open") {
    ...
    return DataRow(
      ...
      cells: [
        ...
        DataCell(cellWrap(e.hmSekarang)),
        ...
        DataCell(
          Center(
            child: InkWell(
              borderRadius: BorderRadius.circular(14),
              onTap: () => showDetail(e), // detail tidak diubah
              child: Container(
                padding: const EdgeInsets.symmetric(
                  horizontal: 14,
                  vertical: 10,
  ...
  // ===== PAGINATION PREMIUM (samakan Bus) =====
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _pair(
                        ...
                        _flatBox("HM Unit saat ini", e.hmSekarang),
                      ),
                      const SizedBox(height: 12),

                      // ROW: Tanggal/Waktu + Shift
                      _pair(
                        context,
                        _flatBox(
                      ...
                      _pair(
                        context,
                        _flatBox("Bocor Radiator", e.opsiItem7),
                        _flatBox(
                      ...
                      _pair(
                        ...
                        _flatBox("Alarm Mundur", e.opsiItem10),
                      ),
                      const SizedBox(height: 12),
                      ...
                      _pair(
                        context,
                        _flatBox(
                          "Kondisi kaca depan, belakang, jendelan & spion)",
                          e.opsiItem11,
                        ),
                        _flatBox(
                      ...
                      _pair(
                        context,
                        _flatBox("Suhu mesin", e.opsiItem13),
                      ...
                      _pair(
                        context,
                        _flatBox("Uji rem fisik", e.opsiItem15),
                        _flatBox(
                          "Alat pelambat / Tetrader Control",
                          e.opsiItem16,
                        ),
                      ),
                      const SizedBox(height: 12),
                      ...
                      _pair(
                        ...
                        _flatBox("Kemudi Darurat", e.opsiItem20),
                      ),
                      const SizedBox(height: 12),

                      _pair(
                        context,
                        _flatBox("Sistem Hidrolik", e.opsiItem21),
                        _flatBox("Silinder Hidrolik", e.opsiItem22),
                      ),
                      const SizedBox(height: 12),

                      _pair(
                        context,
                        _flatBox("Mesin", e.opsiItem23),
                        _flatBox("Sistem Elektrik", e.opsiItem24),
                      ),
                      const SizedBox(height: 12),

                      _pair(
                        context,
                        _flatBox("Load Indicator", e.opsiItem25),
                        _flatBox("Instrumen panel", e.opsiItem26),
                      ),
                      const SizedBox(height: 12),

                      _pair(
                        context,
                        _flatBox("Alat Pemadam Api", e.opsiItem27),
                        _flatBox("Kotak P3K", e.opsiItem28),
                      ),
                      const SizedBox(height: 12),

                      _pair(
                        context,
                        _flatBox("Sabuk keselamatan", e.opsiItem29),
                        _flatBox("Radio Komunikasi", e.opsiItem30),
                      ),
                      const SizedBox(height: 12),

                      _flatBox("SIMPER tersedia & aktif", e.opsiItem31),
                      const SizedBox(height: 12),
                      ...
                      _flatBox("Laporan Temuan Hasil P2H", e.laporanTemuan),
                      const SizedBox(height: 12),
                      _flatBox(
                      ...
                      const SizedBox(height: 12),
                      _flatBox("Jam Tidur Anda", e.jamOperasi),
                      const SizedBox(height: 12),
  ...
  // NOTE: isi detail tetap, hanya box style disamakan dengan Bus (support status badge)
  ...
  // ===== DOKUMEN DIALOG (samakan Bus header gradien + close gradien) =====
  ...
  // ===== preview & download: biarkan logic produksi kamu (aku tidak ubah) =====
