...
class P2HTowerLampResultPage extends StatefulWidget {
  @override
  State<P2HTowerLampResultPage> createState() => _P2HTowerLampResultPageState();
}

class _P2HTowerLampResultPageState extends State<P2HTowerLampResultPage> {
  List<P2HTowerlampModel> allData = [];
  List<P2HTowerlampModel> filtered = [];
  int rowsPerPage = 10;
  int currentPage = 0;
  bool loading = true;
  ...
  Future<void> loadData() async {
    try {
      final data = await P2HTowerlampService.fetchP2HTowerlamp();
      setState(() {
        allData = data;
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("Gagal memuat data P2H Tower Lamp: $e")),
      );
    }
  }

  // ===== SEARCH (samakan DT: contains + ranking) =====
  List<P2HTowerlampModel> _searchResults(String v) {
    final q = v.toLowerCase().trim();
    String safeLower(String? s) => (s ?? "").toLowerCase().trim();

    if (q.isEmpty) return List<P2HTowerlampModel>.from(allData);
    ...
    int rankRow(P2HTowerlampModel e) {
      ...
      final l = safeLower(e.lokasiKerja);
      final h = safeLower(e.hmUnit);
    ...
    int firstIndexRow(P2HTowerlampModel e) {
      final fields = [
        safeLower(e.nama),
        safeLower(e.nrp),
  ...
  List<P2HTowerlampModel> get pageData {
      ...
      child: ClipRRect(
        ...
        child: TextField(
          ...
          decoration: InputDecoration(
            hintText:
                "Cari nama / NRP / perusahaan / department / jabatan / lokasi / HM …",
  ...
  DataRow _rowPremium(P2HTowerlampModel e, int index) {
    ...
    return DataRow(
      ...
      cells: [
        ...
        // ✅ FIX UI ONLY: sebelumnya kamu pakai formatTanggal untuk lokasiKerja (bug visual),
        // sekarang tampilkan apa adanya (data & label tetap).
  ...
  // ===== Files dialog (samakan DT: header gradien + close gradien) =====
  ...
  // ====== DETAIL: ISI & LABEL TETAP (hanya wrapper/pair biar konsisten) ======
  void showDetail(P2HTowerlampModel e) {
                ...
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _pair(
                        context,
                        _flatBox("Kondisi Hose", e.opsiItem3),
                        _flatBox("Kondisi Lampu", e.opsiItem4),
                      ),
                      const SizedBox(height: 12),
                      _pair(
                        context,
                        _flatBox("Filter Udara", e.opsiItem5),
                        _flatBox("Sling Condition", e.opsiItem6),
                      ),
                      const SizedBox(height: 12),
                      _pair(
                        context,
                        _flatBox("Alat Pemadam Api Ringan", e.opsiItem7),
                      ...
                      _pair(
                        context,
                        _flatBox("Kondisi Lampu Menara", e.opsiItem11),
                      ...
                      _pair(
                        context,
                        _flatBox("Skidding", e.opsiItem15),
                        _flatBox("Level BBM", e.opsiItem16),
                      ),
                      const SizedBox(height: 12),
                      _pair(
                        context,
                        _flatBox("Gandengan", e.opsiItem17),
                        _flatBox("Penutup Mesin", e.opsiItem18),
                      ),
                      const SizedBox(height: 12),
                      _flatBox("Kondisi Mur / Baut", e.opsiItem19),
