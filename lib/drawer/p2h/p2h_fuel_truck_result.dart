...
import 'package:safety_apps/models/p2h/p2h_fuel_truck.dart';
...
class P2HFuelTruckResultPage extends StatefulWidget {
  final int? openDetailId;
  const P2HFuelTruckResultPage({super.key, this.openDetailId});
  @override
  State<P2HFuelTruckResultPage> createState() => _P2HFuelTruckResultPageState();
}

class _P2HFuelTruckResultPageState extends State<P2HFuelTruckResultPage>
    with SingleTickerProviderStateMixin {
  List<P2HFuelTruckModel> allData = [];
  List<P2HFuelTruckModel> filtered = [];
  ...
  Future<void> loadData() async {
    try {
      final data = await P2HFuelTruckService.fetchP2HFuelTruck();
      if (!mounted) return;
  ...
  List<P2HFuelTruckModel> _applySearchAndDate() {
  ...
  List<ExcelAccess> get _sortedAccessNewest {
    final list = List<ExcelAccess>.from(
      _accessList.where((a) => a.feature == "p2h_fuel_truck"),
    );
    list.sort((a, b) => b.createdAt.compareTo(a.createdAt));
    return list;
  }

  List<P2HFuelTruckModel> _searchResults(String v) {
    final q = v.toLowerCase().trim();

    String safeLower(String? s) => (s ?? "").toLowerCase().trim();

    if (q.isEmpty) return List<P2HFuelTruckModel>.from(allData);
    ...
    int rankRow(P2HFuelTruckModel e) {
    ...
    int firstIndexRow(P2HFuelTruckModel e) {
      final n = safeLower(e.nama);
      final p = safeLower(e.perusahaan);
      final d = safeLower(e.department);
  ...
  List<P2HFuelTruckModel> get pageData {
                          ...
                          children: [
                            ...
                            ResultDateFilterBar(
                              ...
                              onExportExcel: () async {
                                ...
                                await exportExcelCurrentFilter(
                                  ...
                                  canCurrentUserDownloadExcel:
                                      _canCurrentUserDownloadExcel,
                                  type: ExportType.p2h_fuel_truck,
                                  range: _presetRange(_datePreset),
                                  onNoAccess: () =>
                            ...
                            if (_isAdminOrSuperadmin) ...[
                              ResultExcelAccessPanel(
                                ...
                                isEmpty: _accessList
                                    .where((a) => a.feature == "p2h_fuel_truck")
                                    .isEmpty,
  ...
  Widget _table() {
    ...
    final columns = const [
      ...
      DataColumn2(
        label: Center(child: Text("No Lambung Fuel Truck")),
        fixedWidth: 220,
      ),
      DataColumn2(
        label: Center(child: Text("HM Fuel Truck saat ini")),
        fixedWidth: 200,
      ),
  ...
  DataRow _rowPremium(P2HFuelTruckModel e, int index) {
  ...
  void showDetail(P2HFuelTruckModel e) {
    showModalBottomSheet(
      ...
      builder: (_) {
        return ResultDetailBottomSheet(
          title: "Detail Laporan P2H Fuel Truck",
          ...
          content: Column(
            ...
            children: [
              ...
              pairP2H(
                context,
                flatBoxP2H("No Lambung Fuel Truck", e.noLambungUnit),
              ...
              pairP2H(
                ...
                flatBoxP2H("Alat Pelambat / Retarder Control", e.opsiItem18),
              ),
              const SizedBox(height: 12),
              pairP2H(
                context,
              ...
              pairP2H(
                ...
                flatBoxP2H(
                  "Sign Dilarang Merokok, Bahan Mudah Terbakar dll tersedia",
                  e.opsiItem30,
                ),
              ),
              const SizedBox(height: 12),
              flatBoxP2H("SIMPER Tersedia dan Aktif", e.opsiItem31),
              const SizedBox(height: 12),
              const Padding(
                padding: EdgeInsets.symmetric(vertical: 16),
                child: Divider(thickness: 0.5),
              ...
              pairP2H(
                ...
                flatBoxP2H("Kotak PK3", e.opsiStandarKeselamatan3),
              ),
