...
import 'package:safety_apps/models/p2h/p2h_forklift.dart';
...
class P2HForkliftResultPage extends StatefulWidget {
  final int? openDetailId;
  const P2HForkliftResultPage({super.key, this.openDetailId});
  @override
  State<P2HForkliftResultPage> createState() => _P2HForkliftResultPageState();
}

class _P2HForkliftResultPageState extends State<P2HForkliftResultPage>
    with SingleTickerProviderStateMixin {
  List<P2HForkliftModel> allData = [];
  List<P2HForkliftModel> filtered = [];
  ...
  Future<void> loadData() async {
    try {
      final data = await P2HForkliftService.fetchP2HForklift();
      if (!mounted) return;
  ...
  List<P2HForkliftModel> _applySearchAndDate() {
  ...
  List<ExcelAccess> get _sortedAccessNewest {
    final list = List<ExcelAccess>.from(
      _accessList.where((a) => a.feature == "p2h_forklift"),
    );
    list.sort((a, b) => b.createdAt.compareTo(a.createdAt));
    return list;
  }

  List<P2HForkliftModel> _searchResults(String v) {
    final q = v.toLowerCase().trim();

    String safeLower(String? s) => (s ?? "").toLowerCase().trim();

    if (q.isEmpty) return List<P2HForkliftModel>.from(allData);
    ...
    int rankRow(P2HForkliftModel e) {
    ...
    int firstIndexRow(P2HForkliftModel e) {
      final n = safeLower(e.nama);
      final p = safeLower(e.perusahaan);
      final d = safeLower(e.department);
  ...
  List<P2HForkliftModel> get pageData {
                          ...
                          children: [
                            ...
                            if (_isAdminOrSuperadmin) ...[
                              ResultExcelAccessPanel(
                                ...
                                isEmpty: _accessList
                                    .where((a) => a.feature == "p2h_forklift")
                                    .isEmpty,
  ...
  Widget _table() {
    ...
    final columns = const [
      ...
      DataColumn2(label: Center(child: Text("Jabatan")), fixedWidth: 180),
      DataColumn2(label: Center(child: Text("Department")), fixedWidth: 150),
      DataColumn2(label: Center(child: Text("Perusahaan")), fixedWidth: 200),
      DataColumn2(label: Center(child: Text("Tanggal")), fixedWidth: 150),
      DataColumn2(
        label: Center(child: Text("No Lambung Forklift")),
        fixedWidth: 200,
      ),
      DataColumn2(
        label: Center(child: Text("HM Forklift saat ini")),
        fixedWidth: 200,
      ),
      DataColumn2(label: Center(child: Text("Brand Unit")), fixedWidth: 150),
      DataColumn2(label: Center(child: Text("Lokasi Kerja")), fixedWidth: 150),
      DataColumn2(label: Center(child: Text("Siap Kerja")), fixedWidth: 120),
      DataColumn2(label: Center(child: Text("Dokumen")), fixedWidth: 120),
      DataColumn2(label: Center(child: Text("Detail")), fixedWidth: 120),
    ];
  ...
  DataRow _rowPremium(P2HForkliftModel e, int index) {
  ...
  void showDetail(P2HForkliftModel e) {
    showModalBottomSheet(
      ...
      builder: (_) {
        return ResultDetailBottomSheet(
          title: "Detail Laporan P2H Forklift",
          ...
          content: Column(
            ...
            children: [
              ...
              pairP2H(
                ...
                flatBoxP2H("Shift Kerja", e.brandUnit),
              ),
              const SizedBox(height: 12),
              flatBoxP2H("Shift Kerja", e.lokasiKerja),
              const SizedBox(height: 12),
              ...
              pairP2H(
                ...
                flatBoxP2H("Bocor Radiator atau Pipa Air", e.opsiItem2),
              ),
              const SizedBox(height: 12),
              pairP2H(
                context,
              ...
              pairP2H(
                ...
                flatBoxP2H("Keadaan Body dan Kursi Operator", e.opsiItem12),
              ),
              const SizedBox(height: 12),
              pairP2H(
                context,
                flatBoxP2H("Suhu Mesin", e.opsiItem13),
                flatBoxP2H("Klakson", e.opsiItem14),
              ),
              const SizedBox(height: 12),
              pairP2H(
                context,
                flatBoxP2H("Uji Rem Fisik", e.opsiItem15),
                flatBoxP2H(
              ...
              pairP2H(
                context,
                flatBoxP2H("Reaksi Kemudi", e.opsiItem17),
                flatBoxP2H("Sistem Hidrolik", e.opsiItem18),
              ),
              const SizedBox(height: 12),
              pairP2H(
                context,
                ...
                flatBoxP2H("Mesin", e.opsiItem20),
              ),
              const SizedBox(height: 12),
              pairP2H(
                context,
                flatBoxP2H("Rantai Forklift", e.opsiItem21),
                flatBoxP2H("Kondisi Garpu", e.opsiItem22),
              ),
              const SizedBox(height: 12),
              pairP2H(
                context,
                flatBoxP2H("Sistem Elektrik", e.opsiItem23),
                flatBoxP2H("Tuas Kemudi", e.opsiItem24),
              ),
              const SizedBox(height: 12),
              pairP2H(
                context,
                flatBoxP2H("Tuas Penggeser Garpu", e.opsiItem25),
                flatBoxP2H("Tuas Menaikkan dan Menurunkan Garpu", e.opsiItem26),
              ),
              const SizedBox(height: 12),
              pairP2H(
                context,
                flatBoxP2H("Instrument Panel", e.opsiItem27),
                flatBoxP2H("Alat Pemadam Api", e.opsiItem28),
              ),
              const SizedBox(height: 12),
              pairP2H(
                context,
                flatBoxP2H("Sabuk Keselamatan", e.opsiItem29),
                flatBoxP2H(
              ...
              const SizedBox(height: 12),
              flatBoxP2H("SIO / SIMPER", e.opsiItem31),
