...
class P2HBusResultPage extends StatefulWidget {
  @override
  State<P2HBusResultPage> createState() => _P2HBusResultPageState();
}
...
_StatusKind _kind(String? v) {
  ...
  // Layak/Tidak (item pemeriksaan)
  if (t == "layak" || t == "baik" || t == "ok") return _StatusKind.ok;
  if (t == "tidak")
    return _StatusKind.bad; // (opsional, tapi "tidak" sudah ke-no)

  // Standard keselamatan / masuk tambang
...
class _P2HBusResultPageState extends State<P2HBusResultPage> {
  List<P2HBusModel> allData = [];
  List<P2HBusModel> filtered = [];
  int rowsPerPage = 10;
  int currentPage = 0;
  bool loading = true;
  ...
  // ===== LOAD DATA (samakan CHP) =====
  Future<void> loadData() async {
    try {
      final data = await P2HBusService.fetchP2HBus();
      setState(() {
        allData = data;
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text("Gagal memuat data P2H Bus: $e")));
    }
  }

  // ===== SEARCH mirip CHP (contains + ranking ringan) =====
  List<P2HBusModel> _searchResults(String v) {
    final q = v.toLowerCase().trim();

    String safeLower(String? s) => (s ?? "").toLowerCase().trim();

    if (q.isEmpty) return List<P2HBusModel>.from(allData);
    ...
    final results = allData.where((e) {
      ...
      final noLambung = safeLower(e.noLambungUnit);

      return nama.contains(q) ||
          perusahaan.contains(q) ||
          dept.contains(q) ||
          jabatan.contains(q) ||
          noLambung.contains(q);
    }).toList();
    ...
    int rankRow(P2HBusModel e) {
      final n = safeLower(e.nama);
      final p = safeLower(e.perusahaan);
      final d = safeLower(e.department);
      final j = safeLower(e.jabatan);
      final u = safeLower(e.noLambungUnit);
    ...
    int firstIndexRow(P2HBusModel e) {
      final n = safeLower(e.nama);
      final p = safeLower(e.perusahaan);
      final d = safeLower(e.department);
      final j = safeLower(e.jabatan);
      ...
      if (inJ >= 0) best = inJ < best ? inJ : best;
      if (inU >= 0) best = inU < best ? inU : best;
      ...
      child: ClipRRect(
        ...
        child: TextField(
          ...
          decoration: InputDecoration(
            hintText:
                "Cari nama / perusahaan / department / jabatan / no unit …",
          ...
          children: [
            ...
            DataTable2(
              ...
              columns: const [
                ...
                DataColumn2(
                  label: Center(child: Text("No Lambung Bus")),
                  fixedWidth: 200,
                ),
                DataColumn2(
                  label: Center(child: Text("KM Bus Saat Ini")),
                  fixedWidth: 200,
                ),
                DataColumn2(
  ...
  DataRow _rowPremium(P2HBusModel e, int index) {
    ...
    Widget statusChip(String v) {
      ...
      if (_norm(v) == "open") {
        ...
        return _chip(base, Icons.timelapse_rounded, v);
      }
    ...
    return DataRow(
      ...
      cells: [
        ...
        DataCell(cellWrap(e.kmSekarang)),
  ...
  // ===== PAGINATION PREMIUM (samakan CHP) =====
  ...
  // ===== DETAIL (isi data sama, hanya widget _flatBox jadi support status Ya/Tidak/N/A) =====
  void showDetail(P2HBusModel e) {
                ...
                child: SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _pair(
                        context,
                        _flatBox("No Lambung Bus", e.noLambungUnit),
                        _flatBox("KM Bus saat ini", e.kmSekarang),
                      ),
                      const SizedBox(height: 12),
                      ...
                      _pair(
                        ...
                        _flatBox("Level Air Radiator", e.opsiItem2),
                      ),
                      const SizedBox(height: 12),
                      ...
                      _pair(
                        context,
                        _flatBox("Kondisi Wiper", e.opsiItem5),
                        _flatBox("Ban Serap", e.opsiItem6),
                      ),
                      const SizedBox(height: 12),

                      _pair(
                        context,
                        _flatBox("Kondisi Ban", e.opsiItem7),
                        _flatBox("Tekanan Ban", e.opsiItem8),
                      ),
                      const SizedBox(height: 12),

                      _pair(
                        context,
                        _flatBox("Sekrup Ban", e.opsiItem9),
                        _flatBox("Kelurusan Ban", e.opsiItem10),
                      ),
                      const SizedBox(height: 12),

                      _pair(
                        context,
                        _flatBox("Rem Kaki", e.opsiItem11),
                        _flatBox("Rem Tangan", e.opsiItem12),
                      ),
                      const SizedBox(height: 12),

                      _pair(
                        context,
                        _flatBox("Lampu Depan & Belakang", e.opsiItem13),
                        _flatBox("Lampu Rotary", e.opsiItem14),
                      ),
                      const SizedBox(height: 12),

                      _pair(
                        context,
                        _flatBox("Lampu Sen Kanan & Kiri", e.opsiItem15),
                        _flatBox("Lampu Bahaya", e.opsiItem16),
                      ),
                      const SizedBox(height: 12),

                      _pair(
                        context,
                        _flatBox("Alarm Mundur", e.opsiItem17),
                        _flatBox(
                      ...
                      _pair(
                        ...
                        _flatBox("Suhu Mesin", e.opsiItem20),
                      ),
                      const SizedBox(height: 12),

                      _pair(
                        context,
                        _flatBox("Klakson", e.opsiItem21),
                        _flatBox("Uji Rem Fisik", e.opsiItem22),
                      ),
                      const SizedBox(height: 12),

                      _pair(
                        context,
                        _flatBox("Reaksi Kemudi", e.opsiItem23),
                        _flatBox("Alat Pemadam Api Ringan", e.opsiItem24),
                      ),
                      const SizedBox(height: 12),

                      _pair(
                        context,
                        _flatBox("Dongkrak & Penyangga", e.opsiItem25),
                      ...
                      _pair(
                        context,
                        _flatBox("Scotlight Reflektor", e.opsiItem29),
                        _flatBox("Safety Cone", e.opsiItem30),
                      ),
                      const SizedBox(height: 12),

                      _flatBox("SIMPER", e.opsiItem31),
                      const SizedBox(height: 12),
                      ...
                      _pair(
                        context,
                        _flatBox("Safety Cone / Segitiga", e.opsiKeselamatan1),
                        _flatBox(
                      ...
                      _pair(
                        context,
                        _flatBox("Jack", e.opsiKeselamatan3),
                        _flatBox("Kotak P3K", e.opsiKeselamatan4),
                      ),
                      const SizedBox(height: 12),
                      ...
                      _sectionTitle("Standard Masuk ke Tambang"),

                      _flatBox("Scotlight Reflector", e.opsiMasukTambang1),
                      const SizedBox(height: 12),

                      _pair(
                        context,
                        _flatBox("No Lambung Unit", e.opsiMasukTambang2),
                        _flatBox("Radio Komunikasi", e.opsiMasukTambang3),
                      ),
                      const SizedBox(height: 12),

                      _pair(
                        context,
                        _flatBox("Buggy Whip 4 Mtr", e.opsiMasukTambang4),
                        _flatBox("4WD", e.opsiMasukTambang5),
                      ),
  ...
  // ====== DOKUMEN (logic kamu tetap, header aku samain gradien) ======
