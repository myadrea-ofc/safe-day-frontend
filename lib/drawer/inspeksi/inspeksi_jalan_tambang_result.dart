...
import 'package:safety_apps/widgets/result/chip_apar.dart';
...
class InspeksiJalanTambangResultPage extends StatefulWidget {
  final int? openDetailId;
  const InspeksiJalanTambangResultPage({super.key, this.openDetailId});
  @override
  State<InspeksiJalanTambangResultPage> createState() =>
      _InspeksiJalanTambangResultPageState();
}

class _InspeksiJalanTambangResultPageState
    extends State<InspeksiJalanTambangResultPage>
    with SingleTickerProviderStateMixin {
  List<InspeksiJalanTambangModel> allData = [];
  List<InspeksiJalanTambangModel> filtered = [];
  ...
  void initState() {
    ...
    _loadExcelAccess().then(
      (_) => markExcelAccessSeen(
        role: _role,
        feature: "inspeksi_jalan_tambang",
  ...
  Future<void> loadData() async {
    try {
      final data =
          await InspeksiJalanTambangService.fetchInspeksiJalanTambang();
      if (!mounted) return;
      ...
      if (widget.openDetailId != null && !_openedFromNotif) {
        InspeksiJalanTambangModel? target;
        try {
  ...
  List<InspeksiJalanTambangModel> _applySearchAndDate() {
  ...
  List<InspeksiJalanTambangModel> _searchResults(String v) {
    final q = v.toLowerCase().trim();

    String safeLower(String? s) => (s ?? "").toLowerCase().trim();

    if (q.isEmpty) return List<InspeksiJalanTambangModel>.from(allData);
    ...
    int rankRow(InspeksiJalanTambangModel e) {
    ...
    int firstIndexRow(InspeksiJalanTambangModel e) {
      final n = safeLower(e.nama);
      final p = safeLower(e.perusahaan);
      final d = safeLower(e.department);
      final r = safeLower(e.nrp);
  ...
  List<InspeksiJalanTambangModel> get pageData {
  ...
  List<ExcelAccess> get _sortedAccessNewest {
    final list = List<ExcelAccess>.from(
      _accessList.where((a) => a.feature == "inspeksi_jalan_tambang"),
    );
  ...
  bool get _canCurrentUserDownloadExcel {
    ...
    return _accessList.any(
      (a) =>
          ...
          a.feature == "inspeksi_jalan_tambang" &&
          a.canDownload == true,
    );
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: ResultAppBar(
        title: "Inspeksi Jalan Tambang Results",
                            ...
                            ResultDateFilterBar(
                              ...
                              onExportExcel: () async {
                                ...
                                await exportExcelCurrentFilter(
                                  ...
                                  canCurrentUserDownloadExcel:
                                      _canCurrentUserDownloadExcel,
                                  type: ExportType.inspeksi_jalan_tambang,
                                  range: _presetRange(_datePreset),
                                  onNoAccess: () =>
  ...
  Widget _table() {
    ...
    final columns = const [
      ...
      DataColumn2(label: Center(child: Text("Department")), fixedWidth: 190),
      ...
      DataColumn2(
        label: Center(child: Text("Tersedia APAR Unit")),
        fixedWidth: 200,
      ),
  ...
  DataRow _rowPremium(InspeksiJalanTambangModel e, int index) {
    ...
    return DataRow(
      ...
      cells: [
        ...
        DataCell(Center(child: chipApar(e.apar))),
        DataCell(
          Center(
            child: FotoButton(
              enabled: (e.foto1?.isNotEmpty ?? false) && e.foto1 != "null",
              onTap: () {
        ...
        DataCell(
          Center(
            child: FotoButton(
              enabled: (e.foto2?.isNotEmpty ?? false) && e.foto2 != "null",
              onTap: () {
                FotoPreviewDialogHelper(
                  context: context,
        ...
        DataCell(
          Center(
            child: FotoButton(
              enabled: (e.foto3?.isNotEmpty ?? false) && e.foto3 != "null",
              onTap: () {
                FotoPreviewDialogHelper(
                  context: context,
  ...
  void showDetail(InspeksiJalanTambangModel e) {
    showModalBottomSheet(
      ...
      builder: (_) {
        return ResultDetailBottomSheet(
          title: "Detail Laporan Inspeksi Jalan Tambang",
          ...
          content: Column(
            ...
            children: [
              ...
              buildResultSectionTitle("Kondisi Lapangan"),
              inspeksiFlatBox(
              ...
              const SizedBox(height: 12),
              inspeksiFlatBox("House keeping / kebersihan tambang", e.opsi4),
              const SizedBox(height: 12),
              inspeksiFlatBox(
              ...
              const SizedBox(height: 12),
              inspeksiFlatBox("Rambu-rambu lalu lintas memadai", e.opsi7),
              const SizedBox(height: 12),
              inspeksiFlatBox("Karyawan memakai APD sesuai standar", e.opsi8),
              const SizedBox(height: 12),
              inspeksiFlatBox("Ada rambu petunjuk arah ke PIT", e.opsi9),
              const SizedBox(height: 12),
              inspeksiFlatBox("Setiap kendaraan dilengkapi P3K", e.opsi10),
              const SizedBox(height: 12),
              inspeksiFlatBox(
              ...
              const SizedBox(height: 12),
              inspeksiFlatBox("Klakson digunakan dengan disiplin", e.opsi12),
              const SizedBox(height: 12),
              inspeksiFlatBox("Jalan angkut rata dan bebas batuan", e.opsi13),
              const SizedBox(height: 12),
              inspeksiFlatBox("Lebar jalan angkut sesuai standar", e.opsi14),
              const SizedBox(height: 12),
              inspeksiFlatBox("Jalan angkut ada bundwall & drainase", e.opsi15),
              const SizedBox(height: 12),
              inspeksiFlatBox("Tikungan buta ada cermin cembung", e.opsi16),
              const SizedBox(height: 12),
              inspeksiFlatBox(
              ...
              const SizedBox(height: 12),
              inspeksiFlatBox("Pekerja dewatering memakai pelampung", e.opsi18),
              const SizedBox(height: 12),
              inspeksiFlatBox("Pompa/tower lamp ada pengaman", e.opsi19),
              const SizedBox(height: 12),
              inspeksiFlatBox("Sling dan alat angkat lengkap KIP", e.opsi20),
              const SizedBox(height: 12),
              inspeksiFlatBox("Semua unit punya lampu rotary", e.opsi21),
              const SizedBox(height: 12),
              inspeksiFlatBox("Kondisi Slop aman & bebas batu lepas", e.opsi22),
              const Padding(
                padding: EdgeInsets.symmetric(vertical: 16),
                child: Divider(thickness: 0.5),
              ),
              ...
              const SizedBox(height: 12),
              inspeksiFlatBox(
                "Inspeksi dapat dipertanggungjawabkan?",
                e.statusInspeksi,
              ),
