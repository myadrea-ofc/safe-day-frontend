...
import 'package:safety_apps/models/inspeksi/inspeksi_fasilitas_bbm.dart';
...
class InspeksiFasilitasBBMResultPage extends StatefulWidget {
  final int? openDetailId;
  const InspeksiFasilitasBBMResultPage({super.key, this.openDetailId});
  @override
  State<InspeksiFasilitasBBMResultPage> createState() =>
      _InspeksiFasilitasBBMResultPageState();
}

class _InspeksiFasilitasBBMResultPageState
    extends State<InspeksiFasilitasBBMResultPage>
    with SingleTickerProviderStateMixin {
  List<InspeksiFasilitasBBMModel> allData = [];
  List<InspeksiFasilitasBBMModel> filtered = [];
  ...
  void initState() {
    ...
    _loadExcelAccess().then(
      (_) => markExcelAccessSeen(
        role: _role,
        feature: "inspeksi_fasilitas_bbm",
  ...
  Future<void> loadData() async {
    try {
      final data =
          await InspeksiFasilitasBBMService.fetchInspeksiFasilitasBBM();
      if (!mounted) return;
      ...
      if (widget.openDetailId != null && !_openedFromNotif) {
        InspeksiFasilitasBBMModel? target;
        try {
  ...
  List<ExcelAccess> get _sortedAccessNewest {
    final list = List<ExcelAccess>.from(
      _accessList.where((a) => a.feature == "inspeksi_fasilitas_bbm"),
    );
    list.sort((a, b) => b.createdAt.compareTo(a.createdAt));
    return list;
  }

  List<InspeksiFasilitasBBMModel> _searchResults(String v) {
    final q = v.toLowerCase().trim();

    String safeLower(String? s) => (s ?? "").toLowerCase().trim();

    if (q.isEmpty) return List<InspeksiFasilitasBBMModel>.from(allData);
    ...
    int rankRow(InspeksiFasilitasBBMModel e) {
    ...
    int firstIndexRow(InspeksiFasilitasBBMModel e) {
      final n = safeLower(e.nama);
      final p = safeLower(e.perusahaan);
      final d = safeLower(e.department);
      final r = safeLower(e.nrp);
  ...
  List<InspeksiFasilitasBBMModel> _applySearchAndDate() {
  ...
  List<InspeksiFasilitasBBMModel> get pageData {
  ...
  bool get _canCurrentUserDownloadExcel {
    ...
    return _accessList.any(
      (a) =>
          ...
          a.feature == "inspeksi_fasilitas_bbm" &&
          a.canDownload == true,
    );
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: ResultAppBar(
        title: "Inspeksi Fasilitas BBM Results",
                            ...
                            ResultDateFilterBar(
                              ...
                              onExportExcel: () async {
                                ...
                                await exportExcelCurrentFilter(
                                  ...
                                  canCurrentUserDownloadExcel:
                                      _canCurrentUserDownloadExcel,
                                  type: ExportType.inspeksi_fasilitas_bbm,
                                  range: _presetRange(_datePreset),
                                  onNoAccess: () =>
  ...
  DataRow _rowPremium(InspeksiFasilitasBBMModel e, int index) {
  ...
  void showDetail(InspeksiFasilitasBBMModel e) {
        ...
        return ResultDetailBottomSheet(
          ...
          content: Column(
            ...
            children: [
              ...
              buildResultSectionTitle("Kondisi Fasilitas BBM"),
              inspeksiFlatBox("Bocoran dari Selang / Sambungan", e.opsi1),
              const SizedBox(height: 12),
              inspeksiFlatBox("Tumpahan (di lantai / di tanah)", e.opsi2),
              const SizedBox(height: 12),
              inspeksiFlatBox(
                "Semua selang dilengkapi dengan pucuk pengisian",
                e.opsi3,
              ),
              const SizedBox(height: 12),
              inspeksiFlatBox(
                "Areal bebas dari sampah yang mudah terbakar",
                e.opsi4,
              ),
              const SizedBox(height: 12),
              inspeksiFlatBox(
                "Tanggul dengan lantai kedap cairan (isi maks. + 10%)",
                e.opsi5,
              ),
              const SizedBox(height: 12),
              inspeksiFlatBox(
                "Sistem drainase dalam tanggul (dalam posisi terkunci)",
                e.opsi6,
              ),
              const SizedBox(height: 12),
              inspeksiFlatBox(
                "Instalasi dipagari dengan pintu dan dikunci",
                e.opsi7,
              ),
              const SizedBox(height: 12),
              inspeksiFlatBox("Penerangan area yang mencukupi", e.opsi8),
              const SizedBox(height: 12),
              inspeksiFlatBox(
                "Housekeeping umum / penumpukan & penyimpanan",
                e.opsi9,
              ),
              const SizedBox(height: 12),
              inspeksiFlatBox(
                "Tersedia penampung tetesan (Mobile Units)",
                e.opsi10,
              ),
              const SizedBox(height: 12),
              inspeksiFlatBox(
                "Semua pekerja menggunakan Alat Pelindung Diri",
                e.opsi11,
              ),
              const SizedBox(height: 12),
              inspeksiFlatBox("Saluran pengisian kosong", e.opsi12),
              const SizedBox(height: 12),
              inspeksiFlatBox(
                "Tangga akses untuk tes kedalaman / tersedia tongkat kedalaman tangki",
                e.opsi13,
              ),
              const SizedBox(height: 12),
              inspeksiFlatBox("Pentahanan / Bonding", e.opsi14),
              const SizedBox(height: 12),
              inspeksiFlatBox("Tutup katup Discharge pada tempatnya", e.opsi15),
              const SizedBox(height: 12),
              inspeksiFlatBox(
                "Peralatan pemadam api tersedia dan memadai",
                e.opsi16,
              ),
              const SizedBox(height: 12),
              inspeksiFlatBox(
              ...
              const SizedBox(height: 12),
              inspeksiFlatBox(
                "Terdapat daftar pemberian & penerimaan",
                e.opsi18,
              ),
              const SizedBox(height: 12),
              inspeksiFlatBox("Tersedia pemadam api dan di inspeksi", e.opsi19),
              const SizedBox(height: 12),
              inspeksiFlatBox(
                "Tersedia sign wajib pada area Fuel Station",
                e.opsi20,
              ),
              const SizedBox(height: 12),
              inspeksiFlatBox("Tempat sampah (dengan tutup)", e.opsi21),
              const Padding(
                padding: EdgeInsets.symmetric(vertical: 16),
                child: Divider(thickness: 0.5),
              ),
