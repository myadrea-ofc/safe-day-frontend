...
import 'package:safety_apps/models/inspeksi/inspeksi_chp.dart';
...
class InspeksiCHPResultPage extends StatefulWidget {
  final int? openDetailId;
  const InspeksiCHPResultPage({super.key, this.openDetailId});
  @override
  State<InspeksiCHPResultPage> createState() => _InspeksiCHPResultPageState();
}

class _InspeksiCHPResultPageState extends State<InspeksiCHPResultPage>
    with SingleTickerProviderStateMixin {
  List<InspeksiCHPModel> allData = [];
  List<InspeksiCHPModel> filtered = [];
  ...
  Future<void> loadData() async {
    try {
      final data = await InspeksiCHPService.fetchInspeksiCHP();
      if (!mounted) return;
  ...
  List<InspeksiCHPModel> _applySearchAndDate() {
  ...
  List<ExcelAccess> get _sortedAccessNewest {
    final list = List<ExcelAccess>.from(
      _accessList.where((a) => a.feature == "inspeksi_chp"),
    );
    list.sort((a, b) => b.createdAt.compareTo(a.createdAt));
    return list;
  }

  List<InspeksiCHPModel> _searchResults(String v) {
    final q = v.toLowerCase().trim();

    String safeLower(String? s) => (s ?? "").toLowerCase().trim();

    if (q.isEmpty) return List<InspeksiCHPModel>.from(allData);
    ...
    int rankRow(InspeksiCHPModel e) {
    ...
    int firstIndexRow(InspeksiCHPModel e) {
      final n = safeLower(e.nama);
      final p = safeLower(e.perusahaan);
      final d = safeLower(e.department);
      final r = safeLower(e.nrp);
  ...
  List<InspeksiCHPModel> get pageData {
  ...
  DataRow _rowPremium(InspeksiCHPModel e, int index) {
  ...
  void showDetail(InspeksiCHPModel e) {
        ...
        return ResultDetailBottomSheet(
          ...
          content: Column(
            ...
            children: [
              ...
              buildResultSectionTitle("Kondisi CHP"),
              inspeksiFlatBox(
              ...
              const SizedBox(height: 12),
              inspeksiFlatBox("Jalur & tempat berjalan aman", e.opsi5),
              const SizedBox(height: 12),
              inspeksiFlatBox("Tersedia penutup belt (kerudung)", e.opsi6),
              const SizedBox(height: 12),
              inspeksiFlatBox("Tersedia alat LOTO & tanda bahaya", e.opsi7),
              const SizedBox(height: 12),
              inspeksiFlatBox(
              ...
              const SizedBox(height: 12),
              inspeksiFlatBox("Tersedia Saklar Pembatas Trip", e.opsi9),
              const SizedBox(height: 12),
              inspeksiFlatBox(
              ...
              const SizedBox(height: 12),
              inspeksiFlatBox("Rollers & Idlers semua bekerja", e.opsi12),
              const SizedBox(height: 12),
              inspeksiFlatBox(
              ...
              const SizedBox(height: 12),
              inspeksiFlatBox("Belt bertumpu secara benar", e.opsi15),
              const SizedBox(height: 12),
              inspeksiFlatBox(
              ...
              const SizedBox(height: 12),
              inspeksiFlatBox("Tersedia pemadam api & di inspeksi", e.opsi19),
              const SizedBox(height: 12),
              inspeksiFlatBox("Tersedia peringatan untuk Start Up", e.opsi20),
              const Padding(
                padding: EdgeInsets.symmetric(vertical: 16),
                child: Divider(thickness: 0.5),
              ),
