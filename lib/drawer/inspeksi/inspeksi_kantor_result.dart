...
import 'package:safety_apps/models/inspeksi/inspeksi_kantor.dart';
...
class InspeksiKantorResultPage extends StatefulWidget {
  final int? openDetailId;
  const InspeksiKantorResultPage({super.key, this.openDetailId});
  @override
  State<InspeksiKantorResultPage> createState() =>
      _InspeksiKantorResultPageState();
}

class _InspeksiKantorResultPageState extends State<InspeksiKantorResultPage>
    with SingleTickerProviderStateMixin {
  List<InspeksiKantorModel> allData = [];
  List<InspeksiKantorModel> filtered = [];
  ...
  Future<void> loadData() async {
    try {
      final data = await InspeksiKantorService.fetchInspeksiKantor();
      if (!mounted) return;
  ...
  List<InspeksiKantorModel> _applySearchAndDate() {
  ...
  List<InspeksiKantorModel> _searchResults(String v) {
    final q = v.toLowerCase().trim();

    String safeLower(String? s) => (s ?? "").toLowerCase().trim();

    if (q.isEmpty) return List<InspeksiKantorModel>.from(allData);
    ...
    int rankRow(InspeksiKantorModel e) {
    ...
    int firstIndexRow(InspeksiKantorModel e) {
      final n = safeLower(e.nama);
      final p = safeLower(e.perusahaan);
      final d = safeLower(e.department);
      final r = safeLower(e.nrp);
  ...
  List<InspeksiKantorModel> get pageData {
  ...
  List<ExcelAccess> get _sortedAccessNewest {
    final list = List<ExcelAccess>.from(
      _accessList.where((a) => a.feature == "inspeksi_kantor"),
    );
                            ...
                            ResultDateFilterBar(
                              ...
                              onExportExcel: () async {
                                ...
                                await exportExcelCurrentFilter(
                                  ...
                                  canCurrentUserDownloadExcel:
                                      _canCurrentUserDownloadExcel,
                                  type: ExportType.inspeksi_kantor,
                                  range: _presetRange(_datePreset),
                                  onNoAccess: () =>
  ...
  DataRow _rowPremium(InspeksiKantorModel e, int index) {
  ...
  void showDetail(InspeksiKantorModel e) {
    showModalBottomSheet(
      ...
      builder: (_) {
        return ResultDetailBottomSheet(
          title: "Detail Laporan Inspeksi Kantor",
          ...
          content: Column(
            ...
            children: [
              ...
              buildResultSectionTitle("Kondisi Kantor"),
              inspeksiFlatBox(
              ...
              const SizedBox(height: 12),
              inspeksiFlatBox("Terdapat pencahayaan yang cukup", e.opsi3),
              const SizedBox(height: 12),
              inspeksiFlatBox(
              ...
              const SizedBox(height: 12),
              inspeksiFlatBox("Platform dan jalan diatas ketinggian", e.opsi11),
              const SizedBox(height: 12),
              inspeksiFlatBox(
